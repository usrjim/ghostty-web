...
import { KittyFlagsTracker } from './kitty-flags';
import type {
  IBufferNamespace,
  IBufferRange,
  IDisposable,
...
export class Terminal implements ITerminalCore {
  ...
  // Kitty keyboard protocol state (tracked from program output; see
  // writeInternal). Read by InputHandler to switch key encoding grammar.
  private readonly kittyFlagsTracker = new KittyFlagsTracker();
  ...
  open(parent: HTMLElement): void {
    ...
    try {
      ...
      this.inputHandler = new InputHandler(
        ...
        this.textarea,
        mouseConfig,
        () => this.kittyFlagsTracker.currentFlags
      );
  ...
  private writeInternal(data: string | Uint8Array, callback?: () => void): void {
    ...
    // Track kitty keyboard protocol state from program output (push/pop/set
    // CSI-u sequences). Purely observational; mirrors the wasm's own parser
    // so InputHandler can switch key-encoding grammar when flags are active.
    this.kittyFlagsTracker.scan(data);
